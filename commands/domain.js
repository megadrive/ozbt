...
if( util.checkAccess(args[0], user, args[2], 'moderator') ){
	var spl = args[3].toLowerCase().split(' ');
...
function addDomain(domain, consequence, timeoutTime){
	...
	util.say(args[0], util.getDisplayName(user) + ' -> I will now ' + consequence + ' anyone who posts a link from ' + domain + '.');
}
...
function removeDomain(domain){
	...
	util.say(args[0], util.getDisplayName(user) + ' -> ' + domain + ' removed from list of punishable domains.');
}
